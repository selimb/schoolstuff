...
C           From http://chimeracfd.com/programming/gryphon/fluxroe.html
            real(dp), dimension(:, :), intent(in) :: prim
            real(dp), dimension(3, size(prim, 2)), intent(in) :: w, f
            real(dp), dimension(3, size(prim, 2) - 1) :: f_edge
            real(dp) :: rhoL, sqrhoL, uL, pL, hL, cL,
     &                  rhoR, sqrhoR, uR, pR, hR, cR,
     &                  drho, du, dh, dpp,
     &                  uRL, hRL, rhoRL, cRL, delta, scf, lamb
            real(dp), dimension(3) :: lambdas, dW, phis
            real(dp), dimension(3,3) :: R
            integer :: i, j, k, n
            n = size(prim, 2)
            do i = 1, n - 1
C               Calculate left and right values and delta
                rhoL = prim(1, i)
                rhoR = prim(1, i+1)
                drho = rhoR - rhoL
                sqrhoL = sqrt(rhoL)
                ...
                dpp = pR - pL
                hL = (prim(4, i  ) + pL)/rhoL
                hR = (prim(4, i+1) + pR)/rhoR
                dh = hR - hL
                cL = prim(5, i)
                cR = prim(5, i+1)
C               Equations (42) through (45)
                uRL = (sqrhoL*uL + sqrhoR*uR)/(sqrhoL + sqrhoR)
                hRL = (sqrhoL*hL + sqrhoR*hR)/(sqrhoL + sqrhoR)
                rhoRL = sqrhoL*sqrhoR
                cRL = sqrt( (gam - 1)*(hRL - 0.5*uRL**2) )
C               Equations (46), (50), (51)
                lambdas(1) = uRL
                lambdas(2) = uRL + cRL
                lambdas(3) = uRL - cRL
                delta = du
                do j = 1, 3
                    lamb = lambdas(j)
                    if ( abs(lamb) .lt. delta ) then
                        phis(j) = (lamb**2 + delta**2)/(2*delta)
                    else
                        phis(j) = abs(lamb)
                    end if
                end do
C               Equation (47)
                dW(1) = drho - (dpp/cRL**2)
                dW(2) = du + dpp/(rhoRL*cRL)
                dW(3) = du - dpp/(rhoRL*cRL)
C               Equation (48)
                R(:,1) = 1
                R(1,2) = uRL
                R(2,2) = uRL + cRL
...
C               Equation (49)
                ...
                do j = 1, 3
C                   f_edge(j,i) = f_edge(j,i) - 0.5*sum(phis*dW*R(:,j))
                    do k = 1, 3
                        f_edge(j, i) = f_edge(j, i)
     &                      - 0.5*phis(k)*dW(k)*R(k,j)
                    end do
                end do
